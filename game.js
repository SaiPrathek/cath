...
const ui = Object.fromEntries(['loadingScreen','loadingStatus','retryLoad','titleScreen','storyScreen','storyImage','storyDialogue','storySpeaker','storyBeatDots','storyPortraitCanvas','levelScreen','tutorialScreen','victoryScreen','victoryImage','endingScreen','hud','controls','dialogue','toast','bossHud','actLabel','levelLabel','hearts','cream','rescued','goalPill','bossName','bossBar','storyKicker','storyTitle','storyText','storyQuote','pageNumber','levelKicker','levelTitle','levelDescription','levelMission','portraitCanvas','brandPortrait','titlePretzel','titleCat','titleDonut','hudPretzel','giftPretzel'].map(id=>[id,document.querySelector('#'+id)]));
document.querySelector('#giftLink').href = GIFT_LINK;
const ASSET_ROOT='/cath/assets/pixel/';
const images={};
const ASSET_FILES={sprites:'sprites.png',tiles:'tiles.png',gates:'gates.png',commons:'background-commons-v3.png',factory:'background-sprinkleworks-v3.png',castle:'background-gauntlet-v3.png',story1:'story-1-happy.png',story2:'story-2.png',story3:'story-3-v2.png',story4:'story-4-v2.png',story5:'story-5-v2.png',victory:'story-6-v1.png'};
...
const LEVELS = [
  ...
  {act:'ACT III',name:'The Glazed Gauntlet',desc:'Glazebald’s theatrical fortress, ending in a royal showdown with one extremely dramatic donut.',mission:'Rescue the final two pretzels • Defeat King Glazebald',width:2850,start:{x:70,y:360},exit:2740,theme:'castle',
...
function hitEnemy(e,p){if(e.kind==='king'){e.hp--;e.flash=.18;burst(p.x,p.y,'#f0a2c0',14);shake();tone(170,.08,'square');if(e.phase===1&&e.hp===3){e.phase=2;e.state='roll';queueDialogue([["King Glazebald",null,"Impossible! Initiate rolling majestically!"],["Cat Crumbwell",null,"That’s just rolling."],["King Glazebald",null,"Majestically!"]]);return}if(e.hp<=0)defeatBoss(e);updateHud();return}
...
function bossIntroduction(boss){boss.introShown=true;return boss.kind==='king'?[["King Glazebald",null,"So. You’re the bagel ruining my coronation birthday."],["Cat Crumbwell",null,"It is neither your coronation nor your birthday."],["King Glazebald",null,"It has a banner. That makes it official."],["Cat Crumbwell",null,"The banner is upside down."],["King Glazebald",null,"Then it is officially upside down."]]:[["Sir Sprinkles",null,"Halt! By royal order, you are officially surrounded!"],["Cat Crumbwell",null,"There’s nobody behind me."],["Sir Sprinkles",null,"Unofficially surrounded."]]}
...
function drawUiSprite(canvas,key,scale=.88){if(!canvas||!images.sprites)return;const p=canvas.getContext('2d'),box=SPRITE_FRAMES[key],w=canvas.width,h=canvas.height,size=Math.floor(Math.min(w,h)*scale);p.clearRect(0,0,w,h);p.imageSmoothingEnabled=false;p.drawImage(images.sprites,...box,Math.floor((w-size)/2),Math.floor((h-size)/2),size,size)}
function drawChromeSprites(){drawUiSprite(ui.brandPortrait,'cat.idle.0',1);drawUiSprite(ui.titlePretzel,'pretzel.happy.0',.84);drawUiSprite(ui.titleCat,'cat.idle.0',.94);drawUiSprite(ui.titleDonut,'king.walk.0',.92);drawUiSprite(ui.hudPretzel,'pretzel.happy.0',1);drawUiSprite(ui.giftPretzel,'pretzel.happy.1',.86)}
