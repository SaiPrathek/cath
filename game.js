...
const ui = Object.fromEntries(['loadingScreen','loadingStatus','retryLoad','titleScreen','storyScreen','storyImage','storyDialogue','storySpeaker','storyBeatDots','storyPortraitCanvas','levelScreen','tutorialScreen','victoryScreen','victoryCanvas','endingScreen','hud','controls','dialogue','toast','bossHud','actLabel','levelLabel','hearts','cream','rescued','goalPill','bossName','bossBar','storyKicker','storyTitle','storyText','storyQuote','pageNumber','levelKicker','levelTitle','levelDescription','levelMission','portraitCanvas'].map(id=>[id,document.querySelector('#'+id)]));
...
function bossIntroduction(boss){boss.introShown=true;return boss.kind==='king'?[["King Glazebald","🍩","So. You’re the bagel ruining my coronation birthday."],["Cat Crumbwell","◎","It is neither your coronation nor your birthday."],["King Glazebald","🍩","It has a banner. That makes it official."],["Cat Crumbwell","◎","The banner is upside down."],["King Glazebald","🍩","Then it is officially upside down."]]:[["Sir Sprinkles","🍩","Halt! By royal order, you are officially surrounded!"],["Cat Crumbwell","◎","There’s nobody behind me."],["Sir Sprinkles","🍩","Unofficially surrounded."]]}
...
function drawVictoryArt(){if(!ui.victoryCanvas||!images.sprites)return;const v=ui.victoryCanvas.getContext('2d');v.imageSmoothingEnabled=false;v.clearRect(0,0,320,180);if(images.castle)v.drawImage(images.castle,0,0,960,540,0,0,320,180);v.fillStyle='#251a3270';v.fillRect(0,0,320,180);v.fillStyle='#6c3c67';v.fillRect(0,143,320,37);v.fillStyle='#e8a33c';v.fillRect(0,143,320,5);v.fillStyle='#4b3158';for(let x=0;x<320;x+=24)v.fillRect(x,158+(x/24%2)*7,14,7);const pretzelFrame=Math.floor(performance.now()/150)%4;[22,54,274,302].forEach((x,i)=>drawSprite(`pretzel.happy.${(pretzelFrame+i)%4}`,x-20,104+(i%2)*8,42,42,i>1,1,v));const catFrame=Math.floor(performance.now()/145)%4;drawSprite(`cat.victory.${catFrame}`,76,51,92,92,false,1,v);const kingBox=SPRITE_FRAMES['king.walk.0'];v.save();v.translate(240,125);v.rotate(Math.PI/2);v.drawImage(images.sprites,...kingBox,-38,-38,76,76);v.restore();v.fillStyle='#fff0c2';v.fillRect(207,128,42,8);v.fillRect(216,121,26,7);v.fillStyle='#e8a33c';v.fillRect(263,144,24,6);v.fillRect(268,137,14,7);v.fillStyle='#d94f78';for(let i=0;i<16;i++)v.fillRect((i*43+17)%310,12+(i*29)%88,3+(i%2)*2,5)}
...
function draw(){ctx.imageSmoothingEnabled=false;if(scene==='loading'||scene==='title'||scene==='story'||scene==='levelIntro'||scene==='tutorial'||scene==='victory'){drawBackdrop();if(scene==='victory')drawVictoryArt()}else drawWorld()}
